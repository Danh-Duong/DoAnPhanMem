...
public class EmailUtils {
	...
	public static void sendEmailNoti(String email,String loaihs, String mahs, String noti) {
		Properties props = new Properties();
        props.put("mail.smtp.host", MailConfig.HOST_NAME);
        props.put("mail.smtp.port", MailConfig.PORT);
        props.put("mail.smtp.auth", "true");
        ...
        try {
            ...
            // Đặt nội dung email với định dạng HTML
            String htmlContent = "<h1>Xin chào, "+ email +"</h1>"   
            		+ "<h3>Đăng ký " + loaihs + noti +" </h3>"
            		+ "<h3>Mã hồ sơ của bạn là: <span style='color: green; font-size: 28px; font-weight: 600;'>"+ mahs+"</span></h3>"
            		+ "<p>Xin cảm ơn!</p>";
            message.setContent(htmlContent, "text/html; charset=UTF-8");
